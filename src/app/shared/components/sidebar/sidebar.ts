...
const NAV_SECTIONS: NavSection[] = [
  {
    ...
    items: [
      {
        label: 'Miembros',
        route: '/organizacion/miembros',
        svgPath:
          'M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z',
      },
      {
        label: 'Historial de Cargos',
        route: '/organizacion/historial',
        svgPath:
      ...
      },
      {
        ...
        children: [
          {
            label: 'Centros',
            route: '/organizacion/maestros/centros',
            svgPath:
              'M2.25 21h19.5m-18-18v18m10.5-18v18m6-13.5V21M6.75 6.75h.75m-.75 3h.75m-.75 3h.75m3-6h.75m-.75 3h.75m-.75 3h.75M6.75 21v-3.375c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21M3 3h12m-.75 4.5H21m-3.75 3.75h.008v.008h-.008v-.008Zm0 3h.008v.008h-.008v-.008Zm0 3h.008v.008h-.008v-.008Z',
          },
          {
            label: 'Cargos',
            route: '/organizacion/maestros/cargos',
            svgPath:
...
export class SidebarComponent {
  ...
  protected readonly navSections = computed(() => {
    const permissions = this.authService.currentUserPermissions();
    if (!permissions) {
      return [];
    }
    ...
    for (const section of NAV_SECTIONS) {
      ...
      for (const item of section.items) {
        if (section.title === 'Gestión de Organización') {
          if (permissions.canManageOrganization) {
            filteredItems.push(item);
          }
        } else if (section.title === 'Gestión de Contenidos') {
          if (permissions.canManageWeb) {
            filteredItems.push(item);
          }
        } else if (section.title === 'Seguridad') {
